...
for col in census_columns:
    ...
    data["weighted"] = data.iloc[:, col_number] * data["population"]
    
    X = data["% vaccinated"].values.reshape(-1, 1)  # values converts it into a numpy array
    Y = data["weighted"].values.reshape(-1, 1)  # -1 means that calculate the dimension of rows, but have 1 column
...
# Save and print it.
results.to_csv("../fsa-results-weighted.csv")
print(results)